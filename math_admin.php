...
<head>
    ...
    <title>Electives Math Admin</title>
</head>
...
<?php
        if(isset($_POST['ID'])){
            ...
            header('Location: math_admin.php');
            exit;
        }
?>
...
<body>
  <h2>Изборни дисциплини - ОКН </h2>
  
  <div id="box3">
  <?php
        $connection->beginTransaction();
        $query = "SELECT * FROM electives where type='Математика'  ";
        $stmt = $connection->prepare($query);
        $stmt->execute();
        $connection->commit();
        ...
        while ($record = $stmt->fetch(PDO::FETCH_ASSOC)){
            $number = $record['id'];              
            echo 
            '<form class=form2 method="POST" action="math_admin.php" method="POST">
...
<?php
include "electives_math_admin.html";
?>
</html>