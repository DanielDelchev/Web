...
<head>
    ...
    <title>Electives OKN Admin</title>
</head>
...
<body>
  ...
  <?php
        $connection->beginTransaction();
        $query = "SELECT * FROM electives where type='ОКН'  ";
        $stmt = $connection->prepare($query);
        $stmt->execute();
        $connection->commit();
        ...
        while ($record = $stmt->fetch(PDO::FETCH_ASSOC)){
            $number = $record['id'];              
            echo 
            '<form class=form2 method="POST" action="OKN_admin.php" method="POST">
...
<?php
include "electives_OKN_admin.html";
?>
</html>